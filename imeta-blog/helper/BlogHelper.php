...
class BlogHelper {
    ...
    public function blogDetailsForPostHomePage($db, $urlValue) {
        try {
            // SQL query to retrieve blog post data, assuming you have a table named blog_posts
            $sql = "SELECT * FROM BlogPost WHERE URL = :urlValue ORDER BY PostID DESC";

            // Prepare the statement
            $stmt = $db->prepare($sql);

            // Bind the parameter
            $stmt->bindParam(':urlValue', $urlValue, PDO::PARAM_STR);
